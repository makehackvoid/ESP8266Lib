...
char *
ffp(uint8 res, sint32 v)	// format int as fixed point
{
	...
	if (res >= 1 && res <= 6) {
		...
		d = f[res];
////		os_snprintf(fmt, sizeof(fmt), "%%s%%d.%%0%dd", res);
		os_sprintf (fmt,              "%%s%%d.%%0%dd", res);
////		os_snprintf(buf, sizeof(buf), fmt, sign, v/d, v%d);
		os_sprintf (buf,              fmt, sign, v/d, v%d);
	} else 
