...
esp_err_t ds18b20_search (uint8_t *ids, int nids)
{
	uint8_t	id[8];
	uint8_t crc;

	DbgR (ds18b20_send_command (DS18B20_SEARCH_ROM));
	DbgR (ow_read_bytes (8, id));
	if (id[7] != (crc = onewire_crc8(id, 7)))
		LogR (ESP_FAIL, "bad id crc %02x, read %02x",
			crc, id[7]);
...
//	addr = ow.search(pin)
//		DS18B20_SEARCH_ROM
//	if (NULL == addr) then break
//	if (0x28 == addr[0] || 0x10 == addr[0]) then      // ds18x20
//	ow.reset_search(pin)

//	newire_search_start(&search);
//	addr = onewire_search_next(&search, pin)

//	sensor_count = ds18x20_scan_devices(SENSOR_GPIO, addrs, MAX_SENSORS);

//	ds18b20_send_command (DS18B20_SEARCH_ROM)
//	ow_read_bytes (8, id)
//	if (0x28 == addr[0] || 0x10 == addr[0]) then      // ds18x20
